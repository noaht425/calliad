...
const inputCls =
  'w-full px-4 py-3 text-sm rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 outline-none focus:border-zinc-400 dark:focus:border-zinc-600 transition-colors';
const btnCls =
  'w-full py-3 text-sm font-medium rounded-xl bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 disabled:opacity-50 transition-opacity';
    ...
    <div className="min-h-screen bg-[#fafaf8] dark:bg-[#0a0a0a] flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm space-y-8">
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">Calliad</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">The clever assistant that remembers so you don&apos;t have to.</p>
        </div>
        ...
        {mode === 'password' && (
          <form onSubmit={withPassword} className="space-y-3">
            ...
            <button type="button" onClick={() => { setMode('code-email'); setError(''); }} className="w-full py-2 text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
        ...
        {mode === 'code-email' && (
          <form onSubmit={sendCode} className="space-y-3">
            ...
            <button type="button" onClick={() => { setMode('password'); setError(''); }} className="w-full py-2 text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
              Use a password
            </button>
          </form>
        )}
        ...
        {mode === 'code-enter' && (
          <form onSubmit={verifyCode} className="space-y-3">
            <p className="text-center text-sm text-zinc-500 dark:text-zinc-400 pb-1">
            ...
            <button type="button" onClick={() => { setMode('password'); setCode(''); setError(''); }} className="w-full py-2 text-xs text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300">
              Back
            </button>
          </form>
        )}
