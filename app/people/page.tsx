...
function FriendForm({ draft, onChange, onSubmit, onCancel, submitLabel, saving }: {
  draft: Partial<Friend>;
  onChange: (v: Partial<Friend>) => void;
  onSubmit: () => void;
  onCancel: () => void;
  submitLabel: string;
  saving: boolean;
  ...
  return (
    <div className="p-4 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-3">
      <div className="space-y-1.5">
        ...
        <input className={fieldClass} value={draft.name ?? ''} onChange={(e) => onChange({ ...draft, name: e.target.value })} placeholder="Alex Smith" />
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <input className={fieldClass} type="email" value={draft.email ?? ''} onChange={(e) => onChange({ ...draft, email: e.target.value || null })} placeholder="alex@example.com" />
      </div>
      <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <input className={fieldClass} value={draft.notes ?? ''} onChange={(e) => onChange({ ...draft, notes: e.target.value || null })} placeholder="Met at WWDC 2019…" />
      </div>
...
export default function PeoplePage() {
  return <Suspense><PeopleInner /></Suspense>;
}
...
function PeopleInner() {
  ...
  const [editDraft, setEditDraft] = useState<Partial<Friend>>({});
  ...
  const [importMatch, setImportMatch] = useState<Friend | null>(null); // existing friend with same name
  const [importMerge, setImportMerge] = useState<Partial<Friend>>({}); // merged draft
  ...
  const load = useCallback(async () => {
    ...
    setFetching(true);
    try {
      const res = await fetch('/api/family?relationship=friend', { headers });
  ...
  // Parse ?import= param once friends are loaded
  useEffect(() => {
    ...
    try {
      ...
      const match = friends.find((f) => f.name.toLowerCase().trim() === normName) ?? null;
      setImportMatch(match);
      // Build merged draft: incoming wins for non-empty fields, fall back to existing
      const merged: Partial<Friend> = {
  ...
  async function addFromBrowse(contact: ParsedContact) {
    // Check if already in People by name
    const normName = contact.name.toLowerCase().trim();
    const match = friends.find((f) => f.name.toLowerCase().trim() === normName) ?? null;
    setBrowsing(false);
    setImportContact(contact);
  ...
  async function syncNow() {
    ...
    const res = await fetch('/api/contacts/sync', { method: 'POST', headers });
    if (res.ok) {
      const d = await res.json();
      setSyncMsg(`Synced ${d.synced ?? 0} contacts from iCloud.`);
      load();
    } else {
  ...
  function dismissImport() {
    ...
    setImportMatch(null);
    setImportMerge({});
    // Remove ?import= from URL without navigating
  ...
  async function confirmImport() {
    ...
    if (importMatch) {
      ...
      await fetch('/api/family', {
        method: 'POST',
        headers,
        body: JSON.stringify({ ...importMerge, relationship: 'friend' }),
      });
    }
  ...
  async function addFriend() {
    ...
    const res = await fetch('/api/family', {
      method: 'POST',
      headers,
      body: JSON.stringify({ ...newFriend, relationship: 'friend' }),
    });
    if (res.ok) {
      setNewFriend({});
      setAdding(false);
  ...
  async function removeFriend(id: string) {
    ...
    setFriends((f) => f.filter((m) => m.id !== id));
  }
  ...
  const filtered = friends.filter((f) => {
    if (search) {
      ...
      if (!f.name.toLowerCase().includes(q) && !(f.location_city ?? '').toLowerCase().includes(q)) return false;
    }
    if (filter === 'birthday') return !!f.birthday;
    if (filter === 'upcoming') {
      if (!f.birthday) return false;
      return daysUntil(f.birthday) <= upcomingDays;
    }
    return true;
  });
  ...
  function ImportCard({ contact, match, merge, onMergeChange, onConfirm, onDismiss, saving: isSaving }: {
    ...
    match: Friend | null;
    merge: Partial<Friend>;
    onMergeChange: (v: Partial<Friend>) => void;
    onConfirm: () => void;
    onDismiss: () => void;
    saving: boolean;
  }) {
    // Fields where both sides have different non-empty values → show conflict picker
    type Field = { key: keyof Friend; label: string; existing: string | null; incoming: string | null };
    const fields: Field[] = [
    ...
    return (
      <div className="bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800 rounded-xl p-4 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
              {match ? `Merge with existing contact` : `Add to People`}
            </p>
        ...
        {/* Conflicting fields — let user pick */}
        {conflicts.length > 0 && (
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={onConfirm} disabled={isSaving} className="flex-1 py-2 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-sm font-medium disabled:opacity-40">
            {isSaving ? 'Saving…' : match ? 'Merge' : 'Add to People'}
          </button>
  ...
  return (
    <div className="min-h-screen bg-[#fafaf8] dark:bg-[#0a0a0a]">
      <header className="sticky top-0 z-10 bg-[#fafaf8]/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-zinc-200/60 dark:border-zinc-800/60 px-4 py-3">
        <div className="max-w-xl mx-auto space-y-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 flex-1">People</h1>
            <span className="text-[11px] font-mono text-zinc-400 dark:text-zinc-600">{friends.length}</span>
            <button onClick={syncNow} disabled={syncing} className="text-xs font-medium text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors disabled:opacity-40">
            ...
            <button onClick={() => { setAdding(true); setNewFriend({}); }} className="text-xs font-medium text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors flex items-center gap-1">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <main className="max-w-xl mx-auto px-4 pt-4 pb-32 space-y-2">
        ...
        {syncMsg && (
          <p className="text-xs text-zinc-500 dark:text-zinc-400 text-center py-1">{syncMsg}</p>
        )}
        ...
        {adding && (
          <FriendForm
            ...
            onSubmit={addFriend}
            onCancel={() => { setAdding(false); setNewFriend({}); }}
            submitLabel={saving ? 'Adding…' : 'Add friend'}
            saving={saving}
          />
        )}
        ...
        {fetching ? (
          ...
          <p className="text-center text-zinc-400 dark:text-zinc-600 text-sm py-12">
            {search ? 'No matches.' : 'No people yet — tap Sync or Add.'}
          </p>
        ) : (
          filtered.map((f) => {
            const isExpanded = expandedId === f.id;
            const isEditing = editingId === f.id;
            const days = f.birthday ? daysUntil(f.birthday) : null;
            ...
            const hasDetails = !!(f.email || f.anniversary || f.notes);

            return (
              <div key={f.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                <div className="px-4 py-3 flex items-center gap-3">
                  ...
                  <button
                    onClick={() => hasDetails && setExpandedId(isExpanded ? null : f.id)}
                  ...
                  >
                    ...
                    {f.birthday && (
                      <p className={`text-[11px] font-mono ${soon ? 'text-rose-500 dark:text-rose-400' : 'text-zinc-400 dark:text-zinc-500'}`}>
                        {formatBirthday(f.birthday, f.birth_year)}
                        {days !== null && days <= 30 && (
                  ...
                  {f.location_city && (
                    <span className="text-[10px] text-zinc-400 dark:text-zinc-600 shrink-0 hidden sm:block">{f.location_city}</span>
                  )}
                  <button
                    onClick={() => { setEditingId(f.id); setEditDraft({ name: f.name, email: f.email, location_city: f.location_city, birthday: f.birthday, birth_year: f.birth_year, anniversary: f.anniversary, notes: f.notes }); setExpandedId(null); }}
                ...
                {isExpanded && hasDetails && (
                  <div className="border-t border-zinc-100 dark:border-zinc-800 px-4 py-3 space-y-1.5">
                    {f.email && <p className="text-xs text-zinc-500 dark:text-zinc-400">{f.email}</p>}
                    {f.anniversary && <p className="text-xs text-zinc-500 dark:text-zinc-400">Anniversary: {formatBirthday(f.anniversary, null)}</p>}
                    {f.notes && <p className="text-xs text-zinc-500 dark:text-zinc-400 italic">{f.notes}</p>}
                  </div>
                )}
              </div>
            );
          ...
          <div className="relative bg-white dark:bg-zinc-950 rounded-t-2xl flex flex-col max-h-[80vh]">
            ...
            <div className="overflow-y-auto flex-1">
              {browseLoading ? (
                ...
                const existingNames = new Set(friends.map((f) => f.name.toLowerCase().trim()));
