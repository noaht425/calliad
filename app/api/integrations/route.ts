...
export async function GET(req: NextRequest) {
  ...
  const [gmail, icloudSvc, evCount, schedCount, emCount, contactCount, ctx] = await Promise.all([
    ...
    adminClient.from('contacts').select('id', { count: 'exact', head: true }).eq('user_id', user.id),
...
export async function POST(req: NextRequest) {
  ...
  if (what === 'calendar' || what === 'all') {
    ...
    out.contacts = await syncContacts(user.id).catch((e) => ({ error: String(e) }));
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (service === 'icloud_calendar') {
    ...
    await adminClient.from('contacts').delete().eq('user_id', user.id).eq('source', 'icloud');
  } else {
