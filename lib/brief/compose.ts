...
const COMMON = `Follow the "Morning brief" example in the persona: a short greeting, then today's schedule taken ONLY from the "On the calendar TODAY" list in the Live data block (if that list says "nothing", say the day is clear and do not fill it in from the profile's class times or routines), anything due soon, a birthday/anniversary ONLY if the "Upcoming — people" block lists one (use its date and day-count as given, never estimate one from background profile notes, and say nothing if that block is absent or empty), and anything from the recent conversation or watched mail that needs a decision. Then a one-line weather note for today, and 2 to 3 news headlines from the last day, just the gist, no editorializing, skip any that are trivial. If there's a genuinely notable gaming headline (a real release, a big announcement, not a review or a listicle), add one line for it; otherwise skip gaming entirely. One message; keep it tight. If the day is quiet, say so in one plain line, don't pad it, and don't list anything that isn't in the TODAY list. Match the greeting to the current time of day; don't comment on what time it is.`;
