...
export async function restaurantTasteBlock(userId: string, text: string): Promise<string | undefined> {
  ...
  // what's actually nearby (OpenStreetMap; best-effort, '' if it can't resolve)
  if (rec) {
  ...
  L.push(
    ...
    'Whether he\'d like a specific place: if it\'s on file, lead with his own score and where it sits among his other ratings; if not, estimate from his cuisine averages and highest-rated places (same cuisine / neighbourhood / vibe) and flag it as an estimate. ' +
      'For a recommendation or nearby options: start from his want-to-try list and top-rated places, then use the nearby list above (what physically exists in the area — no ratings) or web search if present, and cross-check every candidate against his own cuisine preferences and scores before naming it. ' +
